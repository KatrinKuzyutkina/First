...
blocks_hit_cnt = 0   # сколько нам удалось подбить блоков
bomb_duration = 5 # каждые bomb_duration пораженных блоков будет кидаться бомба
...
def computer_shoots(set_to_shoot_from): 
    pygame.time.delay(600) # время стрельбы компьютера
    if set_to_shoot_from:
        ...
        return computer_available_to_fire_set
...
def computer_hits_twice(last_hits_list=last_hits_list):
    last_hits_list.sort() #[[a, b]]
...
def get_neighbours(cell_cor, radius=1):
    """
    cell_cor = [cor_x, cor_y]
    возращает список соседей данной клетки в пределах квадрата со стороной 2 * radius + 1
    учитывает выход за границы поля
    нумерация соседей начинается С 1, НЕ С 0
    """
    res = []
    cor_mid_x, cor_mid_y = cell_cor[0], cell_cor[1]
    for x_cor in range(cor_mid_x - radius, cor_mid_x + radius + 1):
        for y_cor in range(cor_mid_y - radius, cor_mid_y + radius + 1):
            if min(x_cor, y_cor) > 0 and max(x_cor, y_cor) <= 10:
                res.append(tuple([x_cor, y_cor]))
    return res

def check_hit(fired_block, bomb_is_ready, computer_ships_working, computer_ships, computer_ships_set)->int:

    hit = False # флаг того, попали мы или нет (изначально мы не попали)
    blocks_hit_cnt = 0
    if bomb_is_ready: # запускаем бомбу
        to_hit = get_neighbours(fired_block) # формируем список с координатами клеток рядом
        for cors in to_hit: # стерляем в каждую клетку из списка
            if check_hit_or_miss(cors, computer_ships_working, False, computer_ships, computer_ships_set):
                hit = True
                blocks_hit_cnt += 1     # увеличиваем счетчик пораженных блоков

    else:
        hit = check_hit_or_miss(fired_block, computer_ships_working, False, computer_ships, computer_ships_set)
        if hit:
            blocks_hit_cnt += 1     # увеличиваем счетчик пораженных блоков
    return blocks_hit_cnt
...
def main():
    ...
    blocks_hit_cnt = 0   # сколько нам удалось подбить блоков
    bomb_duration = 5 # каждые bomb_duration пораженных блоков будет кидаться бомба
    bomb_is_ready = False # флаг готовности бомбы
    while not game_over:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if (left_margin < x < left_margin + 10 * block_size) and (
                    ...
                    func_res = check_hit(fired_block, bomb_is_ready, computer_ships_working, computer.ships, computer.ships_set)
                    hit = func_res > 0
                    blocks_hit_cnt += func_res
                    computer_turn = not hit # в зависимости от попадания смотрим нужно ли передавать ход компьютеру
                    
                    bomb_is_ready = bool(blocks_hit_cnt != 0 and blocks_hit_cnt % bomb_duration == 0) # проверка готовности бомбы
                    ...
                    if hit and bomb_is_ready:
                        show_message_at_rect_center(
                            "ДОПОЛНИТЕЛЬНЫЙ ХОД БОМБА АКТИВИРОВАНА", message_rect_computer)
                    elif hit:
                        show_message_at_rect_center(
                            f"     ДОПОЛНИТЕЛЬНЫЙ ХОД! АКТИВАЦИЯ {(blocks_hit_cnt % bomb_duration)}/{bomb_duration}     ", message_rect_computer)
                    else:
                        show_message_at_rect_center(
                            f"     ВАШ ПОСЛЕДНИЙ ХОД: {LETTERS[fired_block[0]-1] + str(fired_block[1])} АКТИВАЦИЯ {(blocks_hit_cnt % bomb_duration)}/{bomb_duration}     ", message_rect_computer, color=BLACK)
                else:
        ...
        if computer_turn:
            ...
            if computer_turn: 
                show_message_at_rect_center(
                    "ДОПОЛНИТЕЛЬНЫЙ ХОД", message_rect_human)
            else:
                show_message_at_rect_center(
